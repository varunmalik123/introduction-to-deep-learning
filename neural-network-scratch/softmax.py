...
class Softmax(BaseLayer):
	...
	def forward(self,
		...
		softmax_temp = np.zeros([N,C])
		
		for  i in range(N):
			T_iter = np.exp(input_x[i,:] - input_x[i,:].max(keepdims = True))
			to_return = T_iter / T_iter.sum()
			# print(to_return)
			softmax_temp[i,:] = to_return

		softmax_matrix = softmax_temp
